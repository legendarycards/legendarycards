...
const Home: NextPage<Props> = ({ cards, siteConfig }: Props) => {
  ...
  useEffect(() => {
    setFilteredCards(
      cards.filter((card) =>
        card.title.toLocaleLowerCase().includes(search.toLocaleLowerCase()) || card.description.toLocaleLowerCase().includes(search)
      )
    );
  }, [search, cards]);
      ...
      <Container>
        <div className="max-w-screen-md mx-auto ">
          <div className="text-center">
            <div className="relative w-3/5 center mx-auto mb-4">
            ...
            {filteredCards.map((card) => (
              <div key={card.slug}>
                <CardRow listView={true} card={card} search={search} />
              </div>
            ))}
          </div>
        </div>
